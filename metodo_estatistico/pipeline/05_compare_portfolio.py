...
ATTEMPTS_FILE = PIPELINE_DATA / "_stage5_generation_attempts.json"
MIN_PORTFOLIO_ITEMS = 3
ALTERNATIVE_PORTFOLIO_KEYS = (
    "portfolio_proposto",
    "portfolio",
    "categorias_otimizadas",
    "novo_portfolio",
)
...
- COMPLEMENTAR as fixas, nunca duplicar: se uma demanda ja cabe numa categoria fixa ou de encaminhamento, nao crie outra categoria para ela
...
def _merge_obrigatorias(portfolio_llm: list, cats_obrigatorias: list) -> list:
    ...
    for ob in cats_obrigatorias:
        ...
        # (ex: um item fixo marcado como encaminhamento), ainda reconhece e substitui
...
def _normalize_and_merge_recommendation(
    recommendation,
    mandatory_categories: list,
) -> tuple[dict | None, dict]:
    """Normaliza a resposta e aplica as categorias fixas antes da validacao.

    O contrato historico exige ao menos tres categorias validas. A implementacao
    anterior contava apenas a lista bruta da LLM e so depois acrescentava a
    categoria obrigatoria. Isso rejeitava uma resposta com duas categorias
    complementares que se tornaria valida apos o merge deterministico.
    """
    diagnostic = {
        "response_type": type(recommendation).__name__,
        "alternative_key_used": None,
        "items_before_merge": 0,
        "valid_items_before_merge": 0,
        "valid_items_after_merge": 0,
        "minimum_valid_items": MIN_PORTFOLIO_ITEMS,
    }
    if not isinstance(recommendation, dict):
        diagnostic["reason"] = "response_is_not_an_object"
        return None, diagnostic

    normalized = dict(recommendation)
    if "portfolio_otimizado" not in normalized:
        for alternative in ALTERNATIVE_PORTFOLIO_KEYS:
            if alternative not in normalized:
                continue
            value = normalized.pop(alternative)
                ...
                value = [
                    {
                        ...
                        **(
                            ...
                            else {"descricao": str(item)}
                        ),
                    }
                    for key, item in value.items()
                ]
            normalized["portfolio_otimizado"] = value
            diagnostic["alternative_key_used"] = alternative
            break

    portfolio = normalized.get("portfolio_otimizado")
    if not isinstance(portfolio, list):
        diagnostic["reason"] = "portfolio_otimizado_is_not_a_list"
        return None, diagnostic

    diagnostic["items_before_merge"] = len(portfolio)
    diagnostic["valid_items_before_merge"] = sum(
        isinstance(item, dict) for item in portfolio
    )
    merged = _merge_obrigatorias(portfolio, mandatory_categories)
    diagnostic["valid_items_after_merge"] = sum(
        isinstance(item, dict) for item in merged
    )
    normalized["portfolio_otimizado"] = merged

    if diagnostic["valid_items_after_merge"] < MIN_PORTFOLIO_ITEMS:
        diagnostic["reason"] = "portfolio_too_small_after_mandatory_merge"
        return None, diagnostic
...
def _write_attempts(status: str, attempts: list[dict]) -> None:
    """Registra somente metadados agregados; nunca grava a resposta bruta."""
...
def main():
    ...
    for tentativa in range(1, 4):
        try:
            ...
            _write_attempts("RUNNING", historico_tentativas)
            continue

        candidata, diagnostico = _normalize_and_merge_recommendation(
            resposta,
            cats_obrigatorias,
        )
        diagnostico["attempt"] = tentativa
        if diagnostico.get("alternative_key_used"):
            print(
                "[Stage 5] AVISO: LLM usou "
                f"'{diagnostico['alternative_key_used']}' — normalizado para "
                "'portfolio_otimizado'"
            )
        if candidata is not None:
            recomendacao = candidata
            diagnostico["status"] = "valid"
            historico_tentativas.append(diagnostico)
            _write_attempts("PASS", historico_tentativas)
            print(
                "[Stage 5] OK: "
                f"{diagnostico['valid_items_after_merge']} categorias validas "
                "apos merge deterministico "
                f"(tentativa {tentativa})"
            )
            break
        diagnostico["status"] = "invalid"
        historico_tentativas.append(diagnostico)
        _write_attempts("RUNNING", historico_tentativas)
        print(
            f"[Stage 5] Tentativa {tentativa}/3 — resposta invalida: "
            f"{diagnostico['reason']}; "
            f"{diagnostico['valid_items_after_merge']} categorias validas "
            "apos merge; retentando..."
        )
        prompt_tentativa = (
            prompt
            + "\n\n=== CORRECAO OBRIGATORIA DA RESPOSTA ===\n"
            + "A resposta anterior nao cumpriu o contrato estrutural: "
            + str(diagnostico["reason"])
            + ". Gere novamente o objeto JSON completo. "
            + f"O portfolio_otimizado final deve conter pelo menos "
            + f"{MIN_PORTFOLIO_ITEMS} categorias validas, contando as "
            + "categorias fixas fornecidas, e todos os itens devem ser objetos."
        )

    if recomendacao is None:
        _write_attempts("FAIL", historico_tentativas)
        raise RuntimeError(
            "Stage 5 nao obteve portfolio valido apos 3 tentativas; "
            f"diagnostico agregado em {ATTEMPTS_FILE}"
        )
    ...
    output = {
        "metadata": {
            ...
            "stage5_generation_attempts": len(historico_tentativas),
            "minimum_portfolio_items": MIN_PORTFOLIO_ITEMS,
            "mandatory_merge_before_validation": True,
        },
