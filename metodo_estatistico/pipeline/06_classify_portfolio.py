...
# Categorias de encaminhamento fixo: identificadas para
...
# Diretivas alinhadas ao Stage 6 do metodo 2 (mesmo contrato de classificacao para
# justica na comparacao): classificar UM chamado pela intencao, cego a categoria
# antiga do Jira, sem fallback, com confianca e ambiguidade. O que difere e apenas
# a representacao do portfolio (nomes aqui; category_id no metodo 2).
PROMPT_TEMPLATE = """Você é analista de triagem do portal DTI Pesquisa. Classifique UM chamado em exatamente uma categoria do portfólio, pela intenção do chamado. A categoria antiga do Jira não é fornecida e não deve ser usada.
...
Regras:
- Escolha o nome EXATO de uma das categorias acima, apenas o nome, sem a descrição após os dois-pontos.
- ambiguidade deve ser true quando duas categorias forem plausíveis ou faltarem informações.

Responda APENAS com JSON válido:
{{
  "categoria_nova": "nome EXATO de uma das categorias acima",
...
def classify_ticket(summary: dict) -> dict:
    ...
    # CORRECAO METODOLOGICA: falha tecnica (erro de LLM/JSON ou categoria invalida
    # apos retry) NAO vira o catch-all "Nao encontrou o que procurava?". O catch-all
    # e uma categoria curada legitima e so deve receber decisoes genuinas do modelo.
    # Falhas ficam PENDENTES (categoria_nova=None, revisao_recomendada=True), no
    # espirito do metodo atual, sem contaminar uma categoria real com erro tecnico.
    result = None
    for tentativa in range(2):  # até 2 tentativas: normal + retry se categoria inválida
        try:
            ...
            result = {
                "categoria_nova": None,
                "justificativa":  "Falha tecnica de classificacao (LLM/JSON), sem categoria atribuida.",
                "confianca":      "baixa",
                "revisao_recomendada": True,
                "_pendente":      True,
                "_erro":          str(e),
            }
            break  # falha de LLM/JSON não melhora com retry — fica pendente, sem catch-all
        ...
        # Categoria inválida: na primeira tentativa faz retry; na segunda fica pendente
        if tentativa == 0:
            result["_retry_motivo"] = f"Categoria '{cat}' não reconhecida, tentando novamente."
        else:
            result["categoria_nova"] = None
            result["confianca"] = "baixa"
            result["revisao_recomendada"] = True
            result["_pendente"] = True
            result["_aviso"] = f"Categoria '{cat}' não reconhecida após retry, marcada como pendente (sem catch-all)."

    # Alinhado ao metodo 2: baixa confianca ou ambiguidade marcam revisao (sem virar
    # pendente; a categoria escolhida vale, mas entra na amostra de revisao).
    if result is not None and result.get("categoria_nova") and not result.get("_pendente"):
        amb = result.get("ambiguidade") is True
        if amb or str(result.get("confianca", "")).strip().lower() == "baixa":
            result["revisao_recomendada"] = True
...
def main():
    ...
    pendentes  = sum(1 for s in classificados if s.get("_pendente"))
    retentados = sum(1 for s in classificados if "_retry_motivo" in s)
    classificados_ok = sum(1 for s in classificados if s.get("categoria_nova"))
    print(f"[Stage 6] Concluído: {len(classificados)} processados, {classificados_ok} com categoria")
    if pendentes:
        print(f"  {pendentes} PENDENTES (falha tecnica ou categoria invalida apos retry): "
              f"sem categoria, marcados para revisao. NAO viram catch-all.")
    if retentados:
        print(f"  {retentados} resolvidos no retry (categoria inválida na 1ª tentativa)")
    print(f"[Stage 6] Salvo em: {OUTPUT_FILE}")

    from collections import Counter
    # pendentes (categoria_nova=None) ficam fora da distribuicao do portfolio
    cats = Counter(s.get("categoria_nova") for s in classificados if s.get("categoria_nova"))

    # Separa o portfólio analisável dos encaminhamentos fixos.
