...
- **Duas contribuições:**
  ...
  2. **Metodológica** — o **Estudo Comparativo de Métodos de Descoberta** (abaixo).
...
"rev", "v6" ou "hotfix" na narrativa — esses são ruído de execução, registrados apenas no
apêndice técnico.
...
| **Portfólio curado** | O catálogo de serviços adotado pela área — o alvo do estudo. |
...
regras, resultado, pacote). **Não são renomeados** — mudá-los invalidaria o resultado.
...
|---|---|---|
| **1 — Extração** | Lê os CSVs do Jira, limpa HTML/URLs/e-mails, estrutura os campos. | Transformar o export bruto em dados analisáveis. |
| **2 — Interpretação** | A LLM destila a **intenção** de cada chamado (não o texto literal). | Capturar a demanda real e produzir o **insumo comparável** — congelado e compartilhado por todos os métodos. |
| **3 — Descoberta** | Agrupa os chamados por similaridade de intenção. | Descobrir os **grupos naturais de demanda**. É a **variável comparada** (estatístico × agêntico). |
| **4 — Rotulação** | Nomeia e descreve cada grupo. | Tornar os grupos legíveis e acionáveis. |
| **5 — Consolidação** | Monta o portfólio (request types → grupos lógicos). | Transformar clusters em um **catálogo navegável**. |
| **6 — Classificação** | Encaixa cada chamado no portfólio. | Reclassificar o histórico e permitir **medir a aderência** ao alvo curado. |
| **7 — Finalização curada** | A gestão congela o catálogo; a LLM local projeta automaticamente os chamados nele. | Separar decisão de negócio de classificação, sem exigir rótulos humanos por chamado. |

A **curadoria humana** entra depois, como decisão de negócio (visibilidade de serviços,
separação de ambientes) — critérios que os dados não expressam. Ela é **parte do método**,
não um ajuste posterior.

Depois do encerramento do estudo comparativo, o Stage 7 vigente foi
...
2. os Stages 4–6 converteram os grupos em proposta operacional e evidências;
...
e o agregado final do Stage 7. A versão vigente acrescentou depois Acesso a
...
## 8. Onde fica o histórico técnico

A trilha de execução (tentativas, correções e como os *gates* barraram rodadas inválidas)
é **evidência de reprodutibilidade**, mas fica **fora** da narrativa formal. Ela foi
consolidada em `docs/APENDICE_TECNICO.md`; o estado estruturado preserva os detalhes
máquina-legíveis. A monografia não precisa narrar essa saga operacional.
