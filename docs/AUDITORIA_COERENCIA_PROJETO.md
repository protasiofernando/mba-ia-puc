...
2. Antes do Estágio 1, uma regra exata sobre o `Customer Request Type` removeu
...
3. Na formação histórica do portfólio, o Método Estatístico usou embeddings
   `bge-m3` e K-means no Estágio 3; LLMs executaram os Estágios 4–6. O commit
...
|---|---|---|
| Formação | `formacao_portfolio/hpc/job_formar_candidato_estatistico.sh` | executa Estágios 3–6 e gera candidato; não escreve a decisão humana |
...
| Benchmark estatístico | `job_10_m1_legado_llama.sh` | arquitetura estatística completa, Estágios 3–6 |
| Benchmark agêntico | `job_20_m2_nativo.sh` | arquitetura LLM completa, Estágios 3–6 |
...
- removida do README uma tabela de volumes do portfólio curado que não tinha
  um artefato Estágio 7 vigente para sustentá-la;
- removida a afirmação não demonstrada de redução do *catch-all* para 1,3%;
- substituído o SHA provisório do Estágio 2 pelo SHA efetivamente congelado e
...
- incorporada a materialização operacional posterior do Estágio 7: 1.456
...
- Os volumes do Estágio 7 são projeções retrospectivas automáticas no catálogo
